  ...
  return (
    <div className={`${isBorder ? "sm:border-l-2" : "border-0"} `}>
      <div className="w-full bg-guideSectionBg sm:py-2 sm:px-4 rounded-lg my-4 sm:bg-white p-4">
        <h1 className="text-xl text-blue-500 mb-8">
          <span className="bg-blue-500 text-white rounded-md p-1 m-1">
            {blueBgText}
          </span>
          {notBlueBgText}
        </h1>
        ...
        <div className="my-4 grid  sm:grid-cols-1 gap-0 items-center">
          {windowSize.width > 600 ? (
            <>
              <h1 className="text-2xl font-bold sm:text-5xl mb-1">
                {numStats}
              </h1>
              <h1 className="sm:text-md ">{platform}</h1>
            </>
          ) : (
            <h1 className="text-2xl font-bold sm:text-5xl mb-1 mr-1">
              {numStats} <span className="font-normal text-xl">{platform}</span>
            </h1>
          )}
        </div>

        <CommunityStatsButton
